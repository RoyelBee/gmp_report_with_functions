...
everday_sale_df = pd.read_sql_query("""select  right(TRANSDATE, 2) as Date, isnull(sum(QTYSHIPPED),0) as ItemSales from OESalesDetails
                ...
                order by right(TRANSDATE, 2)""", connection)
...
EveryDay_target_df = pd.read_sql_query("""Declare @CurrentMonth NVARCHAR(MAX);
                    ...
                    where YEARMONTH=CONVERT(varchar(6), dateAdd(month,-1,getdate()), 112) and GPMNAME ='Mr. Mohammad 
                    Akhter Alam Khan'""", connection)
...
plt.text(list_index_for_sale + .2, new_array_of_cumulative_sales[list_index_for_sale] + 1,
...
plt.scatter(list_index_for_sale, new_array_of_cumulative_sales[list_index_for_sale], s=60, facecolors='#113d3c',
            edgecolors='white')
plt.text(list_index_for_target - 1.3, cumulative_target_that_needs_to_plot[list_index_for_target] + 6,
